...
# ============================================================
# 【同時予測モデル】道路の「待ち台数」と「最大間隔発生台数目」を
# 1ステップ先（次サイクル）で同時予測する GNN（単一ステップ版）
#   ・★道路1用_予測モデル作成_待ち台数_間隔同時予測.py と同一アーキテクチャ
#   ・入力 : 各ノード [待ち台数(t), 間隔台数目(t)]
#            （待ちは対象道路のみ×DUAL_ALPHA / 間隔は対象道路のみ値を持つ）
#   ・出力 : 対象道路の [待ち台数(t+1), 間隔台数目(t+1)]
# ============================================================
DUAL_ALPHA = 2  # 対象道路ノードの待ち台数特徴量の強調倍率（学習時と一致させること）
...
class DualPredictionGNN(torch.nn.Module):
    def __init__(self, num_node_features=2):
        ...
        self.skip  = torch.nn.Linear(num_node_features, 128)
        self.lin_wait = torch.nn.Linear(128, 1)
        self.lin_int  = torch.nn.Linear(128, 1)
...
def run_prediction_dual(wait_array, interval_value, edge_index,
                        scaler_wait, scaler_int, model_path,
                        target_node_index=0, alpha=DUAL_ALPHA):
    """
    同時予測モデルで「次サイクルの待ち台数 W」「次サイクルの最大間隔発生台数目 N」を予測する。
      wait_array     : 全道路(道路1〜9)の待ち台数(t)  長さ9のリスト
      interval_value : 対象道路の最大間隔発生台数目(t)  スカラ
    戻り値: (W_pred, N_pred)  ※逆正規化済みの実スケール
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 待ち台数を正規化（9列）
    w = np.array(wait_array, dtype=float).reshape(1, -1)
    ...
    col_wait[target_node_index] *= alpha                                 # 学習時と同じ強調
    x_input = torch.cat([col_wait, col_int], dim=1).to(device)          # [9,2]

    model = DualPredictionGNN(num_node_features=2).to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()
    with torch.no_grad():
        out = model(x_input, edge_index.to(device)).cpu().numpy()       # [9,2]
...
# ============================================================
# 【最大間隔発生台数目】ライブ計測ユーティリティ
#   ★到着間隔計測2026_05_28 copy.py と同じ方式:
#     赤期間中に停車した車両の到着間隔を記録し、最大間隔が発生した
#     車両の順番(1始まり)を 1/3 して四捨五入した値を返す（= 1/3 にする処理）。
# ============================================================
def compute_max_interval_order(arrival_records):
    """arrival_records: [{'interval': float}, ...] → 最大間隔発生台数目(= round(順番/3))"""
    if not arrival_records:
        return 0
    max_index = max(range(len(arrival_records)),
                    key=lambda idx: arrival_records[idx]["interval"])
    return int(round((max_index + 1) / 3))


def record_arrival_for_interval(cycle_info, edges, sim_time, stop_speed=0):
    """赤期間中、新規に停車した車両の到着間隔を cycle_info に記録する（参照プログラムと同等）。"""
    stopped_ids = set()
    for edge in edges:
        for veh_id in traci.edge.getLastStepVehicleIDs(edge):
            if traci.vehicle.getSpeed(veh_id) <= stop_speed:
                stopped_ids.add(veh_id)

    stop_time_after_red = round(sim_time - cycle_info["red_start_time"], 3)
    for veh_id in sorted(stopped_ids):
        if veh_id in cycle_info["recorded_vehicle_ids"]:
            continue
        last_stop = cycle_info["last_stop_time"]
        interval = stop_time_after_red if last_stop is None else round(stop_time_after_red - last_stop, 3)
        cycle_info["arrival_records"].append({"interval": interval})
        cycle_info["recorded_vehicle_ids"].add(veh_id)
        cycle_info["last_stop_time"] = stop_time_after_red
...
# 旧モデル（赤信号前半/後半 予測）は廃止。
# 現在は DUAL_MODEL_CONFIG に定義した「同時予測モデル」のみを使用する。


# ============================================================
# 【同時予測モデル】制御対象道路の設定
#   CONTROLLED_ROADS に入れた道路のみ新モデル（待ち台数+最大間隔発生台数目の
#   同時予測）で信号制御する。それ以外の道路は「固定」（予測も制御もしない）。
#   道路1（フェーズ0・主道路）と道路5（フェーズ6・従道路）を同じ方式で制御する。
#   制御対象を増減したい場合は CONTROLLED_ROADS と DUAL_MODEL_CONFIG を編集する。
# ============================================================
CONTROLLED_ROADS = ["1", "5"]   # 道路1・道路5 を同時予測モデルで制御

_PRED_MODEL_DIR = r"C:\Users\Tsukasa\Desktop\研究\予測\予測モデル"
DUAL_MODEL_CONFIG = {
    "1": {
        "model_path":  os.path.join(_PRED_MODEL_DIR, "2. 到着間隔_同時予測", "道路1", "GCN_epoch_best_model.pth"),
        "scaler_wait": os.path.join(_PRED_MODEL_DIR, "2. 到着間隔_同時予測", "道路1", "scaler_wait.pkl"),
        "scaler_int":  os.path.join(_PRED_MODEL_DIR, "2. 到着間隔_同時予測", "道路1", "scaler_interval.pkl"),
        "node_index":  0,                       # 道路1 = ノード0
        # ↓ 到着間隔の計測対象（J交差点・道路1の青/黄フェーズと流入エッジ）
        "green_phase":  0,                      # 道路1（主道路）青フェーズ
        "yellow_phase": 1,                      # 道路1 黄フェーズ（赤サイクル開始判定に使用）
        "edges": ["E24", "-E23", "-E33"],       # 道路1の流入エッジ（TRAFFIC_LIGHT_CONFIG["J"] id="1"）
    },
    "5": {
        "model_path":  os.path.join(_PRED_MODEL_DIR, "2. 到着間隔_同時予測", "道路5", "GCN_epoch_best_model.pth"),
        "scaler_wait": os.path.join(_PRED_MODEL_DIR, "2. 到着間隔_同時予測", "道路5", "scaler_wait.pkl"),
        "scaler_int":  os.path.join(_PRED_MODEL_DIR, "2. 到着間隔_同時予測", "道路5", "scaler_interval.pkl"),
        "node_index":  4,                       # 道路5 = ノード4
        # ↓ 到着間隔の計測対象（J交差点・道路5の青/黄フェーズと流入エッジ）
        "green_phase":  6,                      # 道路5（従道路）青フェーズ
        "yellow_phase": 7,                      # 道路5 黄フェーズ（赤サイクル開始判定に使用）
        "edges": ["E37", "E36"],                # 道路5の流入エッジ（TRAFFIC_LIGHT_CONFIG["J"] id="5"）
    },
}
...
# 信号設定
# ============================================================
# === シミュレーション時間（「時」で直感的に指定）==========================
#   SUMO は 0時(=0秒) から走り始めます。
#     SIM_END_HOUR : 何時まで回すか（時）。 24 → 0時〜24時（丸1日）。
#                    例) 2 にすれば 0時〜2時（2時間ぶん）の短時間テスト。
#     WARMUP_MIN   : 計測・制御を始める前のウォームアップ（分）。
#                    網が車で埋まるまで計測しない準備時間。
#   ↓ ここを書き換えるだけでOK（秒換算は自動）
SIM_END_HOUR = 24      # 終了時刻（時）   ← ここを変更
WARMUP_MIN   = 20      # ウォームアップ（分）← ここを変更

# --- 時間→秒に自動換算（以下は変更不要）---
SIM_END_TIME = int(SIM_END_HOUR * 3600)   # シミュレーション終了（秒）
PREP_TIME    = int(WARMUP_MIN  * 60)      # 計測・制御開始（秒）= ウォームアップ終了
...
ENABLE_EVEN_CYCLE_CANCEL = True  # True => 累積偏差が閾値で調整サイクルを割り込ませる（累積調整方式）
...
prediction_writer.writerow(["回数", "step", "待ち台数測定", "間隔台数目測定", "予測待ち台数W", "予測間隔N", "前半(=N)", "後半(=W-N)"])
...
prediction_writer_road5.writerow(["回数", "step", "待ち台数測定", "間隔台数目測定", "予測待ち台数W", "予測間隔N", "前半(=N)", "後半(=W-N)"])
...
def adjust_signal_A_based_on_prediction(t3, t4, step, prediction_count, pending_counts):
    ...
    try:
        ...
        if not ENABLE_EVEN_CYCLE_CANCEL:
            ...
            print(f"[道路1制御・全サイクル#{cycle_count_road1}] delta={delta} (t3={t3}, t4={t4})")

        elif road1_adjustment_pending:
            # ===== 調整サイクル：累積偏差を OFFSET_ADJUST_PER_CYCLE s で修正 =====
            delta = -OFFSET_ADJUST_PER_CYCLE if cumulative_control_road1 > 0 else OFFSET_ADJUST_PER_CYCLE
            road1_adjustment_pending = False
            cumulative_control_road1 += delta
            adj_cycle_count_road1 += 1
            print(f"✅ [道路1制御・調整#{cycle_count_road1}] delta={delta} (累積={cumulative_control_road1}s)")
            # 1回の補正で足りない場合は追加調整サイクルを予約
            if abs(cumulative_control_road1) >= OFFSET_ADJUST_PER_CYCLE:
                road1_adjustment_pending = True
                print(f"🔔 [道路1] 累積={cumulative_control_road1}s → 追加調整サイクル予定")

        else:
            # ===== 通常サイクル：予測値に基づく制御 =====
            if t3 > t4:
                delta = t3 * 2          # ゲイン×2（道路5と統一）
            elif t3 < t4:
                delta = -t4 * 2         # ゲイン×2（道路5と統一）
            else:
                delta = 0
            delta = max(-OFFSET_ADJUST_PER_CYCLE, min(delta, OFFSET_ADJUST_PER_CYCLE))
            last_phase_delta = delta
            cumulative_control_road1 += delta
            print(f"✅ [道路1制御・通常#{cycle_count_road1}] delta={delta} (t3={t3}, t4={t4}), 累積={cumulative_control_road1}s")

            if abs(cumulative_control_road1) >= OFFSET_ADJUST_THRESHOLD:
                road1_adjustment_pending = True
                print(f"🔔 [道路1] 累積制御={cumulative_control_road1}s → 次サイクル調整予定")
...
def adjust_signal_based_on_prediction_road5(t3, t4, step, prediction_count_road5, pending_counts):
    ...
    try:
        ...
        if not ENABLE_EVEN_CYCLE_CANCEL:
            ...
            last_phase_delta_road5 = delta5
            print(f"[道路5制御・全サイクル#{cycle_count_road5}] delta5={delta5} (t3={t3}, t4={t4})")

        elif road5_adjustment_pending:
            # ===== 調整サイクル：累積偏差を OFFSET_ADJUST_PER_CYCLE s で修正 =====
            delta5 = -OFFSET_ADJUST_PER_CYCLE if cumulative_control_road5 > 0 else OFFSET_ADJUST_PER_CYCLE
            road5_adjustment_pending = False
            cumulative_control_road5 += delta5
            adj_cycle_count_road5 += 1
            print(f"✅ [道路5制御・調整#{cycle_count_road5}] delta5={delta5} (累積={cumulative_control_road5}s)")
            # 1回の補正で足りない場合は追加調整サイクルを予約
            if abs(cumulative_control_road5) >= OFFSET_ADJUST_PER_CYCLE:
                road5_adjustment_pending = True
                print(f"🔔 [道路5] 累積={cumulative_control_road5}s → 追加調整サイクル予定")
            ...
            if t3 > t4:
                ...
                delta5 = 0
            delta5 = max(-OFFSET_ADJUST_PER_CYCLE, min(delta5, OFFSET_ADJUST_PER_CYCLE))
            last_phase_delta_road5 = delta5
            cumulative_control_road5 += delta5
            print(f"✅ [道路5制御・通常#{cycle_count_road5}] delta5={delta5} (t3={t3}, t4={t4}), 累積={cumulative_control_road5}s")

            if abs(cumulative_control_road5) >= OFFSET_ADJUST_THRESHOLD:
                road5_adjustment_pending = True
                print(f"🔔 [道路5] 累積制御={cumulative_control_road5}s → 次サイクル調整予定")
...
def run_simulation(sumocfg_path, log_dir_path, control_signal=True):
    ...
    # === スケーラー読み込み ===
    # === 同時予測モデルのスケーラー読み込み（制御対象道路のみ） ===
    dual_scalers = {}   # road_id -> (scaler_wait, scaler_int)
    for rid in CONTROLLED_ROADS:
        cfg = DUAL_MODEL_CONFIG.get(rid)
        if cfg is None:
            raise ValueError(f"❌ 道路{rid} の DUAL_MODEL_CONFIG が未定義です")
        for key in ("model_path", "scaler_wait", "scaler_int"):
            if not os.path.exists(cfg[key]):
                raise FileNotFoundError(f"❌ 道路{rid} の {key} が存在しません: {cfg[key]}")
        dual_scalers[rid] = (joblib.load(cfg["scaler_wait"]), joblib.load(cfg["scaler_int"]))
        print(f"✅ 道路{rid} 同時予測モデルのスケーラー読み込み完了")
    ...
    # === 到着間隔トラッキング（制御対象道路の最大間隔発生台数目をライブ計測）===
    interval_active = {}                                        # road_id -> 進行中の赤サイクル情報
    interval_last_order = {rid: 0 for rid in CONTROLLED_ROADS}  # 直近に確定した最大間隔発生台数目
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if last_phase_A == total_phases_A - 1 and current_phase_A == 0 and sim_time >= PREP_TIME:
            cycle_counter += 1
            total_stopped = 0
            total_delay   = 0
        ...
        # ============================================================
        # 【到着間隔】制御対象道路の赤期間中、停車車両の到着間隔を記録する
        #   ・赤サイクル開始 : 黄フェーズから抜けた瞬間（参照プログラムと同じ）
        #   ・赤期間中       : 新規停車車両の到着間隔を逐次記録
        #   ・確定（赤終了）  : 下の「赤終了検知」ブロック内で最大間隔発生台数目を算出
        # ============================================================
        if sim_time >= PREP_TIME and last_phase_A is not None:
            for _rid in CONTROLLED_ROADS:
                _cfg = DUAL_MODEL_CONFIG.get(_rid)
                if _cfg is None:
                    continue
                _gp, _yp = _cfg["green_phase"], _cfg["yellow_phase"]
                # 赤サイクル開始：黄フェーズから抜けた瞬間
                if last_phase_A == _yp and current_phase_A != _yp:
                    interval_active[_rid] = {
                        "red_start_time": sim_time,
                        "last_stop_time": None,
                        "recorded_vehicle_ids": set(),
                        "arrival_records": [],
                    }
                # 赤期間中（青フェーズ以外）：到着間隔を記録
                if _rid in interval_active and current_phase_A != _gp:
                    record_arrival_for_interval(interval_active[_rid], _cfg["edges"], sim_time)
            ...
            if last_phase_A != 0 and current_phase_A == 0:
                for rid in ["1", "10"]:
                    ...
                    if rid == "1":
                        ...
                        # 道路1の赤サイクル終了 → 最大間隔発生台数目を確定
                        _ci1 = interval_active.pop("1", None)
                        interval_order_road1 = (
                            compute_max_interval_order(_ci1["arrival_records"])
                            if _ci1 is not None else interval_last_order.get("1", 0)
                        )
                        interval_last_order["1"] = interval_order_road1

                        last_phase_delta_before_prediction = applied_delta_road1 if signal_A_adjusted_once else 0
                        if "1" in CONTROLLED_ROADS and all(latest_data[r] is not None for r in used_road_ids):
                            logger.log_prediction_trigger(latest_data, label="[道路1] ")
                            # 1サイクルの総待ち台数（前半+後半）を各道路で算出（前後半に分けない）
                            waits = [latest_data[r][0] + latest_data[r][1] for r in used_road_ids]
                            _sw, _si = dual_scalers["1"]
                            _mc = DUAL_MODEL_CONFIG["1"]
                            pred_W, pred_N = run_prediction_dual(
                                wait_array=waits,
                                interval_value=interval_order_road1,
                                edge_index=edge_index,
                                scaler_wait=_sw, scaler_int=_si,
                                model_path=_mc["model_path"],
                                target_node_index=_mc["node_index"],
                            )
                            # 予測待ち台数 W を「最大間隔発生台数目 N」で前半/後半に分割
                            pred_W_i = max(0, int(round(pred_W)))
                            pred_N_i = max(0, int(round(pred_N)))
                            front = min(pred_N_i, pred_W_i)        # 前半 = N（総数を超えないよう制限）
                            back  = max(0, pred_W_i - front)        # 後半 = W - N
                            prediction_count += 1
                            logger.log_prediction_result(prediction_count, cnt_t1, diff, front, back, int(sim_time), label="道路1")
                            ...
                            prediction_writer.writerow([
                                prediction_count, int(sim_time),
                                waits[0], interval_order_road1, pred_W_i, pred_N_i, front, back
                            ])
                            # 比較・信号制御は現状のまま（前半 front を t3 / 後半 back を t4 として渡す）
                            if control_signal and CONTROL_MODE == "prediction":
                                adjust_signal_A_based_on_prediction(front, back, int(sim_time), prediction_count, pending_counts)
                            # 実測値を次サイクル入力に使うため latest_data["1"] は計測値のまま据え置く
                            latest_data["1"] = (cnt_t1, diff)
                        else:
            ...
            # ── 従道路（5/6）赤終了: フェーズが 6 になった瞬間 ──────────────
                ...
                for rid in ["5", "6"]:
                    ...
                    if rid == "5":
                        ...
                        # 道路5は「固定」：CONTROLLED_ROADS に "5" がある場合のみ同時予測で制御する。
                        # （現状は "5" 未登録のため予測も制御もせず、フェーズ6は初期値のまま固定）
                        if "5" in CONTROLLED_ROADS and all(latest_data_road5[r] is not None for r in used_road_ids):
                            last_phase_delta_road5_before_prediction = applied_delta_road5 if signal_road5_adjusted_once else 0
                            _ci5 = interval_active.pop("5", None)
                            interval_order_road5 = (
                                compute_max_interval_order(_ci5["arrival_records"])
                                if _ci5 is not None else interval_last_order.get("5", 0)
                            )
                            interval_last_order["5"] = interval_order_road5
                            logger.log_prediction_trigger(latest_data_road5, label="[道路5] ")
                            waits5 = [latest_data_road5[r][0] + latest_data_road5[r][1] for r in used_road_ids]
                            _sw5, _si5 = dual_scalers["5"]
                            _mc5 = DUAL_MODEL_CONFIG["5"]
                            pred_W5, pred_N5 = run_prediction_dual(
                                wait_array=waits5, interval_value=interval_order_road5,
                                edge_index=edge_index, scaler_wait=_sw5, scaler_int=_si5,
                                model_path=_mc5["model_path"], target_node_index=_mc5["node_index"],
                            )
                            pred_W5_i = max(0, int(round(pred_W5)))
                            pred_N5_i = max(0, int(round(pred_N5)))
                            front5 = min(pred_N5_i, pred_W5_i)
                            back5  = max(0, pred_W5_i - front5)
                            prediction_count_road5 += 1
                            logger.log_prediction_result(prediction_count_road5, cnt_t1, diff, front5, back5, int(sim_time), label="道路5")
                            prediction_writer_road5.writerow([
                                prediction_count_road5, int(sim_time),
                                waits5[0], interval_order_road5, pred_W5_i, pred_N5_i, front5, back5
                            ])
                            if control_signal and CONTROL_MODE == "prediction":
                                adjust_signal_based_on_prediction_road5(front5, back5, int(sim_time), prediction_count_road5, pending_counts)
                            latest_data_road5["5"] = (cnt_t1, diff)
                        else:
                            # 固定運用：予測・制御なし（フェーズ6は変更しない）
                            interval_active.pop("5", None)
                            latest_data_road5["5"] = (cnt_t1, diff)
                            ...
                            if tag == "t1":
                                ...
                                for t1_time, (prev_count, diff_exp) in list(first_half_storage[road_id].items()):
                                    if abs(record_time - t1_time - diff_exp) <= 1:
                                        ...
                                        # 旧モデル（赤信号前半/後半）の予測・制御はここで行っていたが廃止。
                                        # J交差点の道路1・道路5 は同時予測モデル（赤終了検知ブロック）で制御する。
                                        matched = True
                                        break
