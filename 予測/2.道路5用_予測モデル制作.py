...
# ======== 道路5（ノード4）を予測対象とした学習ループ ========
while (rmse_t3 > 1.2 or rmse_t4 > 1.2):
    ...
    for epoch in range(200):
        ...
        for batch in train_loader:
            ...
            # ======== 変更点①：ノード4（道路5）を予測対象に ========
            out_target = out[:, 4, 0]  # shape: [batch_size]  ← 0 から 4 に変更
            y_target   = y[:, 4, 0]    # shape: [batch_size]  ← 0 から 4 に変更

            # === ノード4（道路5）の入力 (t, t+1) を取得 ===
            x4 = batch.x.view(-1, node_num, 1)[:, 4, 0]  # ← 0 から 4 に変更

            # 入力が小さいか判定（最大値 <= 2）
            small_input_mask = (x4 <= 2).float()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # ======== 変更点①（val）：ノード4（道路5）を予測対象に ========
                out_target = out[:, 4, 0]  # ← 0 から 4 に変更
                y_target   = y[:, 4, 0]    # ← 0 から 4 に変更
    ...
    with torch.no_grad():
        for batch in test_loader:
            batch = batch.to(device)
            out = model(batch.x, batch.edge_index)
            out = out.view(-1, node_num, 2)  # t+2, t+3 の2ステップ分を保持
            y = batch.y.view(-1, node_num, 2)

            test_predictions.append(out.cpu().numpy())
            test_ground_truth.append(y.cpu().numpy())
    ...
    test_ground_truth[:, :, 1] = scaler.inverse_transform(test_ground_truth[:, :, 1])

    print("逆正規化・四捨五入前の予測データ：", test_predictions.shape)
    print("逆正規化・四捨五入前のテストデータ：", test_ground_truth.shape)

    # === スライド展開（flatten） ===
    # build_daywise_dataset で 2ステップずつスライドしているため、
    # t+2, t+3 の予測を時系列順に並べ直す
    ...
    print("スライド展開後の予測データ形状：", flattened_pred.shape)
    print("スライド展開後の正解データ形状：", flattened_true.shape)

    # === RMSEの計算（flattenedデータのノード4・道路5） ===
    pred = flattened_pred[:, 4]
    true = flattened_true[:, 4]
    ...
    if rmse_t3 <= 1.1 and rmse_t4 <= 1.2:
        print(f"[達成] 終了条件達成！t3 RMSE={rmse_t3:.4f}, t4 RMSE={rmse_t4:.4f}")
        break
    else:
        print(f"[継続] 継続学習: 前半RMSE={rmse_t3:.4f}, 後半RMSE={rmse_t4:.4f}")

    # === CSV出力（スライド展開後データ使用） ===
    output_csv_path = r"C:\Users\Tsukasa\Desktop\研究\予測\予測結果_比較.csv"

    pred_series = flattened_pred[:, 4]
    true_series = flattened_true[:, 4]
    ...
    prediction_count += 1
    print("最大予測値（ノード4・道路5）：", np.max(pred_series))
...
# === スライド展開後データから道路5（ノード4）の予測列を取得 ===
pred_series = flattened_pred[:, 4].astype(int)
...
for i in range(0, num_steps - 1, 2):
    cycle1 = i + 1
    cycle2 = i + 2
    pred1 = int(pred_series[i])
    pred2 = int(pred_series[i + 1])
...
loss_plot_path = r"C:\Users\Tsukasa\Desktop\研究\予測\Loss曲線_best_model.png"
...
# # === グラフ描画用の長さ取得 ===
# test_long = flattened_pred.shape[0]  # flattenedされたt3, t4系列長

# # 時間帯フィルタに応じて x 軸ラベルを設定
# if USE_TIME_FILTER:
...
#     _num_hours = 24
#     _hour_offset = 0
# cycles_per_hour = test_long / _num_hours
...
# # 保存・表示
# plt.savefig(f'C:\\Users\\Tsukasa\\Desktop\\研究\\予測\\相関係数0.5以上\\道路10_11除く_予測波形_交互線色_{rmse}.png')
# plt.show()
...
# # === 保存・表示 ===
# save_dir = r'C:\Users\Tsukasa\Desktop\研究\予測\相関係数0.5以上'
...
# plt.savefig(f'C:\\Users\\Tsukasa\\Desktop\\研究\\予測\\相関係数0.5以上\\道路10_道路11を除く_予測波形_{rmse}.png')
# plt.show()
...
# # === 保存・表示 ===
# plt.savefig(r'C:\Users\Tsukasa\Desktop\研究\予測\相関係数0.5以上\道路10_11除く_拡大波形_色分け_点付き2.png')
# plt.show()
...
# # SUMOネットワークファイルの読み込み
# net = readNet("C:/Users/Tsukasa/Desktop/研究/予測/toyama_shouwa_offset_smz.net.xml")
