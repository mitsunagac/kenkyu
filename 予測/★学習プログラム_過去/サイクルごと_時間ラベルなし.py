...
answer_csv_path = r"C:\\Users\\tslab\\Desktop\\予測\\共三参照_正解データ.csv"
...
def build_daywise_dataset(data_array, labels, edge_index, device):
    ...
    for day in range(num_days):
        ...
        for t in range(start, end - 3, 2):  # t, t+1 から t+2, t+3 を予測
            ...
            # # 時間ラベル付与（それぞれに）
            # x_t1 = add_time_label_to_features(x_t1, labels[t])
            # x_t2 = add_time_label_to_features(x_t2, labels[t+1])

            # 入力 = [ノード数, 2, 7] → Flatten → [ノード数, 14]
            ...
            y = torch.cat([y_t3, y_t4], dim=1)  # [ノード数, 2]
...
while(rmse > 1.0):
    ...
    for epoch in range(200):  # 例として100エポック
        ...
        for batch in train_loader:
            ...
            out_target = out[:, 0, :]  # shape: [batch_size, 2]
            y_target   = y[:, 0, :]    # shape: [batch_size, 2]
    ...
    # === RMSEの計算（ノード0だけ） ===
    # === RMSE（ノード0, t3とt4を1本にまとめて） ===
    pred_t3 = test_predictions[:, 0, 0]
    true_t3 = test_ground_truth[:, 0, 0]
    pred_t4 = test_predictions[:, 0, 1]
    true_t4 = test_ground_truth[:, 0, 1]

    # RMSE 個別計算
    rmse_t3 = np.sqrt(np.mean((pred_t3 - true_t3) ** 2))
    rmse_t4 = np.sqrt(np.mean((pred_t4 - true_t4) ** 2))

    # 最終 RMSE（t3, t4 の誤差を合算して平均）
    rmse = np.sqrt((np.sum((pred_t3 - true_t3) ** 2) + np.sum((pred_t4 - true_t4) ** 2)) / (len(pred_t3) + len(pred_t4)))

    # 四捨五入・0.5未満を0扱い
    test_predictions = np.round(test_predictions)
    test_predictions[np.abs(test_predictions) < 0.5] = 0
    test_ground_truth = np.round(test_ground_truth)
    ...
    with open(output_csv_path, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        for i in range(flattened_pred.shape[0]):
            writer.writerow([i + 1, flattened_true[i, 0], flattened_pred[i, 0]])

    prediction_count += 1
    print("最大予測値（ノード0）：", np.amax(flattened_pred[:, 0]))
...
for i in range(0, num_cycles - 1, 2):  # 2ステップずつ
    cycle1 = i + 1
    cycle2 = i + 2
    pred1 = int(flattened_pred[i, 0])
    pred2 = int(flattened_pred[i + 1, 0])
...
plt.grid(which="both", axis="both")
plt.minorticks_on()
plt.xticks(hour_ticks[8:10], hour_labels[8:10], rotation=45)
...
# === 保存・表示 ===
plt.savefig(r'C:\Users\tslab\Desktop\予測\相関係数0.5以上\道路10_11除く_拡大波形_色分け_点付き.png')
plt.show()
