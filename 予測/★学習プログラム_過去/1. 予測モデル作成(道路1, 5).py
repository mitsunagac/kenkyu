...
# ===============================
    ...
    if rmse_first <= 1.5 and rmse_second <= 1.5: 
      print(f"🎯 終了条件達成！前半RMSE={rmse_first:.4f}, 後半RMSE={rmse_second:.4f}") 
      break 
    else: 
      print(f"⚙️ 継続学習: 前半RMSE={rmse_first:.4f}, 後半RMSE={rmse_second:.4f}")
...
for i in range(0, num_cycles - 1, 2):  # 2ステップずつ
    cycle1 = i + 1
    cycle2 = i + 2
    pred1 = int(flattened_pred[i, 4])
    pred2 = int(flattened_pred[i + 1, 4])
...
with open(csv_output_path, mode='r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        if row:
            pred_labels.append(int(row[0]))  # ラベル列
