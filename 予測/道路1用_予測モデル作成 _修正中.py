...
train_data_csv = pd.read_csv(r"C:\Users\Tsukasa\Desktop\研究\予測\新環境待ち台数データ_12.22.csv", encoding='cp932')
...
answer_csv_path = r"C:\Users\Tsukasa\Desktop\研究\予測\新環境正解データ_12.22.csv"
...
model_output_dir = os.path.join(BASE_DIR, "予測モデル", f"{target_road_label} 予測モデル")
os.makedirs(model_output_dir, exist_ok=True)

prediction_detail_csv_path = os.path.join(BASE_DIR, f"予測結果_{target_road_label}.csv")
csv_output_path = os.path.join(BASE_DIR, f"予測結果_大小比較_{target_road_label}.csv")
scaler_path = os.path.join(model_output_dir, "scaler.pkl")
loss_plot_path_t3 = os.path.join(BASE_DIR, f"Loss曲線_{target_road_label}_t3モデル.png")
loss_plot_path_t4 = os.path.join(BASE_DIR, f"Loss曲線_{target_road_label}_t4モデル.png")
t3_plot_path = os.path.join(BASE_DIR, f"予測波形_{target_road_label}_t3.png")
t4_plot_path = os.path.join(BASE_DIR, f"予測波形_{target_road_label}_t4.png")
...
# === Dataset 作成（phase別）===
def build_phase_dataset(data_array, edge_index, device, target_node_index, phase, alpha=2):
    ...
    for day in range(num_days):
        ...
        for t in range(range_start, range_end - 3, 2):
            input_idx  = t + phase        # phase=0: t,   phase=1: t+1
            target_idx = t + phase + 2    # phase=0: t+2, phase=1: t+3
            if target_idx >= data_array.shape[1]:
                continue

            x_value = torch.tensor(data_array[:, input_idx], dtype=torch.float).unsqueeze(1).to(device)
            x_value[target_node_index] *= alpha
            phase_feature = torch.full_like(x_value, float(phase))
            x = torch.cat([x_value, phase_feature], dim=1)
            y = torch.tensor(data_array[:, target_idx], dtype=torch.float).unsqueeze(1).to(device)
...
train_loader_t3 = DataLoader(build_phase_dataset(train_data,      edge_index, device, target_node_index, phase=0), batch_size=batch_size, shuffle=False)
val_loader_t3   = DataLoader(build_phase_dataset(validation_data, edge_index, device, target_node_index, phase=0), batch_size=batch_size, shuffle=False)
test_loader_t3  = DataLoader(build_phase_dataset(test_data,       edge_index, device, target_node_index, phase=0), batch_size=batch_size, shuffle=False)

train_loader_t4 = DataLoader(build_phase_dataset(train_data,      edge_index, device, target_node_index, phase=1), batch_size=batch_size, shuffle=False)
val_loader_t4   = DataLoader(build_phase_dataset(validation_data, edge_index, device, target_node_index, phase=1), batch_size=batch_size, shuffle=False)
test_loader_t4  = DataLoader(build_phase_dataset(test_data,       edge_index, device, target_node_index, phase=1), batch_size=batch_size, shuffle=False)
...
def predict_phase(model, test_loader):
    model.eval()
    preds, truths = [], []
    with torch.no_grad():
...
while rmse_t3 > 1.2:
    ...
    pred_t3, true_t3 = predict_phase(model_t3, test_loader_t3)
...
while rmse_t4 > 1.3:
    ...
    pred_t4, true_t4 = predict_phase(model_t4, test_loader_t4)
...
num_cycles = pair_count * 2

for i in range(0, num_cycles - 1, 2):  # 2ステップずつ
    cycle1 = i + 3
    cycle2 = i + 4
    pred1 = int(pred_t3[i // 2])
    pred2 = int(pred_t4[i // 2])
...
# === losの記録 ===
# === 精度評価 ===
comparison_count = min(len(pred_labels), len(true_labels))
correct = sum(p == t for p, t in zip(pred_labels[:comparison_count], true_labels[:comparison_count]))
...
ax1.plot(range(half_len), true_first,  color="blue", linewidth=1.5, label="Actual (t1→t3)")
ax1.plot(range(half_len), pred_first,  color="red",  linewidth=1.5, linestyle="--", label="Predicted (t1→t3)")
...
ax2.plot(range(half_len), true_second, color="blue", linewidth=1.5, label="Actual (t2→t4)")
ax2.plot(range(half_len), pred_second, color="red",  linewidth=1.5, linestyle="--", label="Predicted (t2→t4)")
