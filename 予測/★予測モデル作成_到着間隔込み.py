...
# ============================================================================
# 「待ち台数」と「最大間隔発生台数目（到着間隔）」を同時予測するモデル（単一ステップ版）
# ----------------------------------------------------------------------------
# ★予測対象の道路は 道路1 / 道路5 を選択可能。
#   「最大間隔発生台数目_学習データ.csv」には 道路1・道路5 の2列があり、
#   選んだ道路の到着間隔（間隔台数目）を予測対象にする。
#
# ★学習器の機構（GCNConv×3 + スキップ接続 / 学習率スケジュール / early stopping /
#   RMSE 未達なら再学習する while ループ）は同時予測スクリプトと同一。
#
# ★予測構造（単一ステップ）:
#     各道路の「一つの値(t)」から「次の値(t+1)」を予測する。
#       入力 : ノード特徴量 [待ち台数(t), 間隔台数目(t)]   (num_node_features = 2)
#               待ち台数 は対象道路ノードのみ ×ALPHA / 間隔台数目 は対象道路ノードのみ値
#       出力 : 対象道路の [待ち台数(t+1), 間隔台数目(t+1)]    (2値)
#
# ★入力データ:
#     待ち台数_学習データ.csv          : 全道路(道路1〜9)の待ち台数        [N, 9]
#     最大間隔発生台数目_学習データ.csv : 道路1・道路5 の最大間隔発生台数目 [N, 2]
...
#   TARGET_ROAD = "道路1"  または  TARGET_ROAD = "道路5"
# 実行時に第1引数を渡した場合は、その値が優先されます（例: python ... 5）。
# ※ 間隔台数目データに存在する道路のみ選択可能（現状: 道路1 / 道路5）。
# =========================
TARGET_ROAD = "道路1"
...
# === データパス（★学習用データ フォルダ） ===
# 間隔台数目が「道路1・道路5 の2列」になっているのは 3. のフォルダ。
DATA_DIR = os.path.join(BASE_DIR, "★学習用データ", "3. 到着間隔_学習データ_20260627 (道路1, 5)")
wait_csv_path     = os.path.join(DATA_DIR, "待ち台数_学習データ.csv")            # 全道路の待ち台数 (9列)
interval_csv_path = os.path.join(DATA_DIR, "最大間隔発生台数目_学習データ.csv")  # 道路1・道路5 の間隔台数目 (2列)
adjacency_path    = os.path.join(BASE_DIR, "新環境_隣接行列.csv")

# === データ読み込み ===
#   待ち台数 CSV は cp932 (Shift-JIS), ヘッダ = 道路1〜道路9
#   間隔台数目 CSV は utf-8-sig (BOM 付き), ヘッダ = 道路1, 道路5
...
LAMBDA_SHAPE = 0.5   # 損失における「分布の形（ばらつき）」項の重み（間隔台数目に適用）
WAIT_RMSE_TH = 2.0   # 待ち台数 RMSE の終了しきい値
INT_RMSE_TH  = 2.0   # 間隔台数目 RMSE の終了しきい値
MAX_TRIALS   = 20    # 再学習の最大試行回数

# === 対象ノードの特定（待ち台数グラフ上のノード index） ===
...
# === 間隔台数目データから、対象道路の列を選択 ===
# 間隔データのヘッダ（例: 道路1, 道路5）から target_road_id に一致する列を取り出す。
interval_road_labels = [str(c) for c in interval_data_csv.columns]
interval_road_ids = [extract_road_id(lbl) for lbl in interval_road_labels]
if target_road_id not in interval_road_ids:
    raise ValueError(
        f"対象道路{target_road_id}は間隔台数目データに含まれていません。"
        f"選択可能: {interval_road_ids}"
    )
interval_col_index = interval_road_ids.index(target_road_id)
print(f"間隔台数目 使用列: {interval_road_labels[interval_col_index]} "
      f"(列 index: {interval_col_index})")

# 成果物（モデル・スケーラー・予測結果CSV・グラフ）はすべてこのフォルダに出力する
model_output_dir = os.path.join(BASE_DIR, "予測モデル", "2. 到着間隔_同時予測", target_road_label)
...
df_train_int = interval_data_csv.iloc[:train_long, [interval_col_index]]
df_val_int   = interval_data_csv.iloc[train_long:train_long + val_long, [interval_col_index]]
df_test_int  = interval_data_csv.iloc[train_long + val_long:train_long + val_long + test_long, [interval_col_index]]
...
#               待ち台数 は対象道路ノードのみ ×ALPHA / 間隔台数目 は対象道路ノードのみ値
#   y : [1, 2]  対象道路の [待ち台数(t+1), 間隔台数目(t+1)]
def build_daywise_dataset(wait_arr, int_arr, edge_index, device, target_node_index, alpha=ALPHA):
    ...
    for day in range(num_days):
        ...
        for t in range(range_start, range_end - 1):
            ...
            i_t = torch.zeros((node_num, 1), dtype=torch.float)                 # 間隔は対象道路のみ
...
learning_rates = [0.001] * 100 + [0.0005] * 50 + [0.0001] * 50
...
def distribution_shape_loss(pred, true):
    """ pred, true: [B,1]（バッチ内の予測/正解の集合）。
        『分布の形（ばらつき）』を一致させる損失。
        MSE/MAE は1台ずつの誤差しか見ないため、全体が平均値に張り付いた
...
def dual_loss(out_target, y_target, criterion):
    """ out_target: [B,2], y_target: [B,2]  各列 = [待ち台数, 間隔台数目]。
        待ち台数・間隔台数目それぞれ MSE + MAE。さらに間隔台数目には
        『分布の形（ばらつき）』損失を LAMBDA_SHAPE で混ぜ、平均値張り付きを防ぐ。"""
    ...
    # 間隔台数目の「分布の形（ばらつき）」を本物に近づける項
    shape_int = distribution_shape_loss(out_target[:, 1:2], y_target[:, 1:2])
    return loss_wait + LAMBDA_INT * loss_int + LAMBDA_SHAPE * shape_int
