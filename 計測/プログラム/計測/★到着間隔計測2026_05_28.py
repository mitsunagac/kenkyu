...
generated_vehicle_dir = os.path.join(vehicle_generation_data_root, "★車両データ", "6月28日作成 (24hデータ)", "生成された車")
...
organized_output_root = r"C:\Users\Tsukasa\Desktop\研究\計測\output\計測結果"
work_output_root = os.path.join(organized_output_root, "_作業用")

output_dir_base = os.path.join(work_output_root, "待ち台数_前半")
output_dir_base_2 = os.path.join(work_output_root, "待ち台数_後半")
arrival_interval_output_dir = os.path.join(organized_output_root, "到着間隔", "車両別")
arrival_cycle_summary_output_dir = os.path.join(organized_output_root, "到着間隔", "サイクル集計")
max_arrival_interval_summary_output_dir = os.path.join(organized_output_root, "到着間隔", "最大到着間隔まとめ")
max_interval_order_output_dir = os.path.join(organized_output_root, "到着間隔", "最大間隔発生台数目")
...
# ファイル統合設定
# ======================
merged_output_dir = os.path.join(organized_output_root, "待ち台数_前後半統合")
merged_output_dir_1 = os.path.join(work_output_root, "待ち台数_比較")

machi_folder = output_dir_base
output_filename = "all_days_待ち_統合.csv"

input_dir_base_3 = merged_output_dir_1
output_dir_base_3 = os.path.join(work_output_root, "遅れ時間")
merged_output_dir_3 = os.path.join(work_output_root, "遅れ時間", "統合")
...
def run_simulation_with_signal_logging(day, warmup_time=0):
    ...
    arrival_output_dir = os.path.join(arrival_interval_output_dir, f"day{day}")
    arrival_summary_output_dir = os.path.join(arrival_cycle_summary_output_dir, f"day{day}")
    max_arrival_summary_output_dir = os.path.join(max_arrival_interval_summary_output_dir, f"day{day}")
    max_interval_order_output_dir_day = os.path.join(max_interval_order_output_dir, f"day{day}")
    os.makedirs(output_dir_1, exist_ok=True)
    os.makedirs(output_dir_2, exist_ok=True)
    os.makedirs(arrival_output_dir, exist_ok=True)
    os.makedirs(arrival_summary_output_dir, exist_ok=True)
    os.makedirs(max_arrival_summary_output_dir, exist_ok=True)
    os.makedirs(max_interval_order_output_dir_day, exist_ok=True)
    ...
    arrival_csv_files = {}
    arrival_writers = {}
    arrival_summary_csv_files = {}
    arrival_summary_writers = {}
    max_arrival_summary_path = os.path.join(max_arrival_summary_output_dir, f"最大到着間隔_day{day}.csv")
    max_arrival_summary_file = open_csv_with_fallback(max_arrival_summary_path, encoding='utf-8-sig')
    max_arrival_summary_writer = csv.writer(max_arrival_summary_file)
    max_arrival_summary_writer.writerow([
        "道路ID",
        "サイクル数",
        "停車台数",
        "最大到着間隔(s)",
    ...
    max_interval_order_path = os.path.join(max_interval_order_output_dir_day, f"最大間隔発生台数目_day{day}.csv")
    max_interval_order_file = open_csv_with_fallback(max_interval_order_path, encoding='utf-8-sig')
    max_interval_order_writer = csv.writer(max_interval_order_file)
    max_interval_order_writer.writerow(["道路ID", "サイクル数", "最大間隔発生台数目"])
    ARRIVAL_TARGETS = {
        "1": {"green_phase": 0, "yellow_phase": 1},
        "5": {"green_phase": 3, "yellow_phase": 4},
    }

    def get_arrival_target(config):
        return ARRIVAL_TARGETS.get(str(config['id']))

    def is_arrival_target(config):
        return get_arrival_target(config) is not None
    ...
    for tl_id, configs in TRAFFIC_LIGHT_CONFIG.items():
        for i, config in enumerate(configs):
            ...
            if not is_arrival_target(config):
                continue

            arrival_path = os.path.join(arrival_output_dir, filename)
            arrival_f = open_csv_with_fallback(arrival_path, encoding='utf-8-sig')
            arrival_w = csv.writer(arrival_f)
            arrival_w.writerow(["サイクル数", "赤信号変化後停車時刻(s)", "到着間隔(s)"])
            arrival_csv_files[(tl_id, i)] = arrival_f
            arrival_writers[(tl_id, i)] = arrival_w

            summary_path = os.path.join(arrival_summary_output_dir, filename)
            summary_f = open_csv_with_fallback(summary_path, encoding='utf-8-sig')
            ...
            arrival_summary_csv_files[(tl_id, i)] = summary_f
            arrival_summary_writers[(tl_id, i)] = summary_w
    ...
    red_cycle_counts_1 = {key: 0 for key in arrival_writers}
    active_red_cycles_1 = {}

    def write_cycle_summary(key, cycle_info):
        road_id = cycle_info["road_id"]
        arrival_records = cycle_info["arrival_records"]
        if not arrival_records:
            arrival_summary_writers[key].writerow([
                cycle_info["cycle"],
                0,
                "",
                "",
            ...
            max_interval_order_writer.writerow([
                road_id,
                cycle_info["cycle"],
                0
            ])
        ...
        max_record = max(arrival_records, key=lambda record: record["interval"])
        max_index = arrival_records.index(max_record)
        vehicles_after_max = len(arrival_records) - max_index - 1
        max_interval_order = round((max_index + 1) / 3)
        average_interval = round(
            sum(record["interval"] for record in arrival_records) / len(arrival_records),
            3
        )
        arrival_summary_writers[key].writerow([
            cycle_info["cycle"],
            len(arrival_records),
            arrival_records[0]["stop_time"],
            arrival_records[-1]["stop_time"],
            max_record["interval"],
            max_record["gap_start"],
            max_record["gap_end"],
            average_interval,
            vehicles_after_max
        ])
    ...
    def record_arrival_intervals(key, config, sim_time):
        cycle_info = active_red_cycles_1.get(key)
        ...
        for veh_id in sorted(stopped_vehicle_ids):
            if veh_id in cycle_info["recorded_vehicle_ids"]:
                continue

            last_stop_time = cycle_info["last_stop_time"]
            if last_stop_time is None:
                arrival_interval = stop_time_after_red
                gap_start = 0
            else:
                arrival_interval = round(stop_time_after_red - last_stop_time, 3)
                gap_start = last_stop_time

            arrival_writers[key].writerow([
                cycle_info["cycle"],
                stop_time_after_red,
                arrival_interval
            ])
            cycle_info["arrival_records"].append({
                "stop_time": stop_time_after_red,
                "interval": arrival_interval,
                "gap_start": gap_start,
                "gap_end": stop_time_after_red
            })
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for tl_id, configs in TRAFFIC_LIGHT_CONFIG.items():
            try:
                ...
                print(f"⚠ Error at {tl_id} - CONFIG 1 phase/state: {e}")
                continue
            ...
            for i, config in enumerate(configs):
                try:
                    ...
                    key = (tl_id, i)
                    arrival_target = get_arrival_target(config)

                    if (
                        key in arrival_writers
                        and sim_time >= warmup_time
                        and arrival_target is not None
                        and prev_phase == arrival_target["yellow_phase"]
                        and current_phase != arrival_target["yellow_phase"]
                    ):
                        red_cycle_counts_1[key] += 1
                        active_red_cycles_1[key] = {
                            "road_id": str(config["id"]),
                            "cycle": red_cycle_counts_1[key],
                    ...
                    if (
                        ...
                        and prev_phase != arrival_target["green_phase"]
                        and current_phase == arrival_target["green_phase"]
                    ):
                        cycle_info = active_red_cycles_1.pop(key, None)
                        if cycle_info is not None:
                            write_cycle_summary(key, cycle_info)
                    ...
                    if key in arrival_writers:
                        record_arrival_intervals(key, config, sim_time)
    ...
    for key, cycle_info in list(active_red_cycles_1.items()):
        write_cycle_summary(key, cycle_info)
    active_red_cycles_1.clear()
    ...
    for f in csv_files_2.values():
        f.close()
    for f in arrival_csv_files.values():
        f.close()
    for f in arrival_summary_csv_files.values():
        f.close()
    max_arrival_summary_file.close()
    max_interval_order_file.close()

    traci.close()
    print(f"✅ Day {day}: 待機車両記録を完了しました → {output_dir_1} / {output_dir_2}")
    print(f"✅ Day {day}: 到着間隔記録を完了しました → {arrival_output_dir}")
    print(f"✅ Day {day}: 到着間隔サイクル集計を完了しました → {arrival_summary_output_dir}")
    print(f"✅ Day {day}: 最大到着間隔まとめを完了しました → {max_arrival_summary_path}")
    print(f"✅ Day {day}: 最大間隔発生台数目を完了しました → {max_interval_order_path}")
...
# ファイル統合(最大間隔発生台数目)
# ======================
def combine_interval_order_vertically(days, max_rows=720):
    output_folder = os.path.join(max_interval_order_output_dir, "統合")
    os.makedirs(output_folder, exist_ok=True)
    final_output_path = os.path.join(output_folder, "最大間隔発生台数目_統合.csv")

    all_rows = []
    for day in range(1, days + 1):
        file_path = os.path.join(
            max_interval_order_output_dir, f"day{day}", f"最大間隔発生台数目_day{day}.csv"
        )
        if not os.path.isfile(file_path):
            print(f"スキップ: ファイルが見つかりません -> {file_path}")
            continue
        df = pd.read_csv(file_path, encoding='utf-8-sig')
        if "道路ID" in df.columns:
            df["道路ID"] = df["道路ID"].astype(str)
            ...
            target_road_ids = [road_id for road_id in ["1", "5"] if road_id in pivot_df.columns]
            pivot_df = pivot_df.reindex(columns=target_road_ids)
            pivot_df.columns = [f"道路{road_id}" for road_id in pivot_df.columns]
            pivot_df = pivot_df.reset_index(drop=True).iloc[:max_rows]
            all_rows.append(pivot_df)
        else:
            df = df.iloc[:max_rows]
            all_rows.append(df[["最大間隔発生台数目"]].rename(columns={"最大間隔発生台数目": "道路1"}))
...
#     ]
    ...
    if RESUME_FROM_CHECKPOINT:
        ...
        for d in range(1, days + 1):
            marker = os.path.join(
                max_interval_order_output_dir, f"day{d}", f"最大間隔発生台数目_day{d}.csv"
            )
            if os.path.isfile(marker):
                try:
                    marker_df = pd.read_csv(marker, encoding='utf-8-sig')
                    has_road5 = (
                        "道路ID" in marker_df.columns
                        and marker_df["道路ID"].astype(str).eq("5").any()
                    )
                except Exception:
                    has_road5 = False
    ...
    for day in range(start_day, days + 1):
        ...
        print(f"=== Day {day} : 最大間隔発生台数目_統合 ===")
        combine_interval_order_vertically(day) # 最大間隔発生台数目 統合(全日)
