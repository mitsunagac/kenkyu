...
netfile = r"C:\Users\tslab\Desktop\町モデル\shin_11.12shuusei.net.xml" #新環境(11/25)
...
net = sumolib.net.readNet(r"C:\Users\tslab\Desktop\町モデル\toyama_shouwa_offset_smz.net.xml")
...
top_level_weights_by_time = [
    [0.4, 0.3, 0.3],  # 深夜
    [0.4, 0.3, 0.35],  # 早朝
    [0.45, 0.28, 0.27],  # 朝 (ピーク)
    [0.42, 0.3, 0.28],  # 昼
    [0.41, 0.3, 0.29],  # 昼過ぎ
    [0.45, 0.3, 0.25],  # 夕方 (ピーク)
    [0.4, 0.4, 0.3],  # 夜
]


# 第2層サブグループの重み
group1_weights = [0.54, 0.46]  # 南内の比率
group2_weights = [0.6, 0.4]  # 北内の比率
...
TRAFFIC_LIGHT_CONFIG = {
    ...
    "F": [{"id": "4", "edges":  ["E159", "E158"], "red": "yyyrrrrryyyrrrrr", "green": "rrrrrrrrrrrrrrrr"},],
    ...
    "K": [{"id": "8", "edges": ["E36"], "red": "rrrrrryy", "green": "rrrrrrrr"},],
}

TRAFFIC_LIGHT_CONFIG_2 = {
    "A": [{"id": "1", "edges": ["DtoA"], "red": "rrrryyyyyrrrryyyyy", "green": "rrrrrrrrrrrrrrrrrr"},],
    ...
    "I": [{"id": "5", "edges": ["E26"], "red": "yyyyrrryyyyrrr", "green": "rrrrrrrrrrrrrr"},],
    "J": [{"id": "9", "edges": ["M-1toJ"], "red": "rrrryyyyyrrrryyyyy", "green": "rrrrrrrrrrrrrrrrrr"},
...
output_dir_base_2 = r"C:\Users\tslab\Desktop\町モデル\output\待ち台数_中間"
...
# ファイル統合設定
# ======================
merged_output_dir = r"C:\Users\tslab\Desktop\町モデル\output\待ち台数_遅れ時間"
merged_output_dir_1 = r"C:\Users\tslab\Desktop\町モデル\output\待ち台数_比較"

machi_folder = r"C:\Users\tslab\Desktop\町モデル\output\待ち台数"
output_filename = "all_days_待ち_統合.csv"

input_dir_base_3 = r"C:\Users\tslab\Desktop\町モデル\output\待ち台数_比較"  # CSVファイルが入っているフォルダに変更してください
output_dir_base_3 = r"C:\Users\tslab\Desktop\町モデル\output\遅れ時間"
merged_output_dir_3 = r"C:\Users\tslab\Desktop\町モデル\output\遅れ時間\統合"
...
def run_simulation_with_signal_logging(day, warmup_time=0):
    ...
    csv_files_1 = {}  # TRAFFIC_LIGHT_CONFIG 用
    writers_1 = {}

    csv_files_2 = {}  # TRAFFIC_LIGHT_CONFIG_2 用
    writers_2 = {}
    ...
    pending_counts_1 = {}  # {(信号ID, index): 計測予定時刻} 2秒後用
    pending_counts_2 = {}  # {(信号ID, index): 計測予定時刻} 24秒後用
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
                    ...
                    if key in pending_counts_1 and sim_time >= pending_counts_1[key]:
                        ...
                        try:
                            ...
                            if id_int in [1, 2, 3, 4, 9]:
                                total_count = math.ceil(total_count / 3)
                            elif id_int in [5, 6, 7, 8]:
        ...
        # ==== TRAFFIC_LIGHT_CONFIG_2 用 記録（24秒後） ====
        for tl_id, configs in TRAFFIC_LIGHT_CONFIG_2.items():
            for i, config in enumerate(configs):
                try:
                    current = traci.trafficlight.getRedYellowGreenState(tl_id)
                    ...
                    if sim_time >= warmup_time and prev == config['red'] and current == config['green']:
                        pending_counts_2[key] = sim_time + 21
...
#     time_settings = [
#         (1300, 4, 5), #準備   0    ~ 1300
#         (3600, 2, 5), #1     1300  ~ 4900
#         (3600, 2, 5), #2     4900  ~ 8500
#         (3600, 2, 5), #3     8500  ~ 12100
#         (3600, 1, 4), #4     12100 ~ 15700
#         (3600, 1, 4), #5     15700 ~ 19300
#         (3600, 1, 3), #6     19300 ~ 22900
#         (3600, 1, 3), #7     22900 ~ 26500
#         (3600, 1, 2), #8 大  26500 ~ 30100
#         (3600, 1, 1), #9 大  30100 ~ 33700
#         (3600, 1, 2), #10    33700 ~ 37300
#         (3600, 1, 3), #11    37300 ~ 40900
#         (3600, 1, 3), #12    40900 ~ 44500
#         (3600, 1, 3), #13    44500 ~ 48100 (切り替わり)
#         (3600, 1, 3), #14    48100 ~ 51700
#         (3600, 1, 3), #15    51700 ~ 55300
#         (3600, 1, 2), #16    55300 ~ 58900
#         (3600, 1, 2), #17    58900 ~ 62500
#         (3600, 1, 1), #18大  62500 ~ 66100
#         (3600, 1, 2), #19大  66100 ~ 69700
#         (3600, 1, 3), #20    69700 ~ 73300
#         (3600, 1, 3), #21    73300 ~ 76900
#         (3600, 1, 3), #22    76900 ~ 80500
#         (3600, 1, 4), #23    80500 ~ 84100
#         (3600, 1, 4), #24    84100 ~ 87700
#     ]
